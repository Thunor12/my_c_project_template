// Template-repo build script — dogfoods every optional dependency in config/features/.
// Bootstrap: gcc -o nob nob.template.c  (or: NOB_SRC=nob.template.c ./nob.sh)
//
// Generated projects use the skeleton nob.c instead.
...
int main(int argc, char **argv)
{
        ...
        if (0 == strcmp(arg, "setup"))
        {
            if (argc > 0)
            {
                ...
                else
                {
                    ...
                    nob_log(NOB_ERROR, "Add new deps under config/features/<name>/ and wire them here.");
                    return 1;
                }
            }
            else
    ...
    if (do_setup_sqlite)
    {
        ...
        nob_log(NOB_ERROR, "sqlite not enabled — run scripts/enable-sqlite.sh");
        return 1;
#else
    ...
    if (do_setup_teapot)
    {
        ...
        nob_log(NOB_ERROR, "teapot not enabled — run scripts/enable-teapot.sh");
        return 1;
#else
    ...
    if (do_examples)
    {
        ...
        if (!built_any)
        {
            nob_log(NOB_WARNING, "No examples built — enable deps: scripts/enable-*.sh");
        }
